...
- You must always use relative imports for imports within the `misskey` (internal imports must
  be relative).
...
Also you can see the latest formatting rules on the [analysis_options.yaml](https://github.com/misskey-dart/misskey/blob/main/analysis_options.yaml)!
...
So `misskey` uses the following standard prefixes depending on endpoint characteristics.
